...
typedef struct
{
    ...
    uint32_t FMC_DataSetupTime; /*!< Defines the number of HCLK cycles to configure
                                     ...
                                     @note This parameter is used for SRAMs, ROMs and asynchronous multiplexed NOR Flash
                                  memories. */
...
#define IS_FMC_MEMORY(MEMORY) \
    (((MEMORY) == FMC_MemoryType_SRAM) || ((MEMORY) == FMC_MemoryType_PSRAM) || ((MEMORY) == FMC_MemoryType_NOR))
/**
 * @}
 */
...
#define IS_FMC_WAIT_SIGNAL_ACTIVE(ACTIVE) \
    (((ACTIVE) == FMC_WaitSignalActive_BeforeWaitState) || ((ACTIVE) == FMC_WaitSignalActive_DuringWaitState))
/**
 * @}
 */
...
#define IS_FMC_ROWBITS_NUMBER(ROW) \
    (((ROW) == FMC_RowBits_Number_11b) || ((ROW) == FMC_RowBits_Number_12b) || ((ROW) == FMC_RowBits_Number_13b))
...
#define IS_FMC_SDMEMORY_WIDTH(WIDTH) \
    (((WIDTH) == FMC_SDMemory_Width_8b) || ((WIDTH) == FMC_SDMemory_Width_16b) || ((WIDTH) == FMC_SDMemory_Width_32b))
...
#define IS_FMC_CAS_LATENCY(LATENCY) \
    (((LATENCY) == FMC_CAS_Latency_1) || ((LATENCY) == FMC_CAS_Latency_2) || ((LATENCY) == FMC_CAS_Latency_3))
...
#define IS_FMC_SDCLOCK_PERIOD(PERIOD) \
    (((PERIOD) == FMC_SDClock_Disable) || ((PERIOD) == FMC_SDClock_Period_2) || ((PERIOD) == FMC_SDClock_Period_3))
...
#define IS_FMC_READPIPE_DELAY(DELAY) \
    (((DELAY) == FMC_ReadPipe_Delay_0) || ((DELAY) == FMC_ReadPipe_Delay_1) || ((DELAY) == FMC_ReadPipe_Delay_2))
...
#define IS_FMC_GETFLAG_BANK(BANK) \
    (((BANK) == FMC_Bank2_NAND) || ((BANK) == FMC_Bank3_NAND) || ((BANK) == FMC_Bank4_PCCARD) || \
        ((BANK) == FMC_Bank1_SDRAM) || ((BANK) == FMC_Bank2_SDRAM) || ((BANK) == (FMC_Bank1_SDRAM | FMC_Bank2_SDRAM)))
