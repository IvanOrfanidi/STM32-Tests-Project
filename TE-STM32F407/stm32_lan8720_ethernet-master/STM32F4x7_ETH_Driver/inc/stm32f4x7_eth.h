...
typedef struct
{
    ...
    */
    ...
    uint32_t ETH_ChecksumOffload; /*!< Selects or not the IPv4 checksum checking for received frame payloads'
    ...
    */
    ...
    uint32_t ETH_ForwardUndersizedGoodFrames; /*!< Enables or disables the Rx FIFO to forward Undersized frames (frames
    ...
    uint32_t ETH_SecondFrameOperate; /*!< Selects or not the Operate on second frame mode, which allows the DMA to
...
#define ETH_DMAPTPRxDesc_PTPMT_PdelayReq_Announce \
    ((uint32_t)0x00000500) /* PdelayReq message (peer-to-peer transparent clock) or Announce message (Ordinary or \
                                Boundary clock) */
#define ETH_DMAPTPRxDesc_PTPMT_PdelayResp_Manag \
    ((uint32_t)0x00000600) /* PdelayResp message (peer-to-peer transparent clock) or Management message (Ordinary or \
                                Boundary clock)  */
#define ETH_DMAPTPRxDesc_PTPMT_PdelayRespFollowUp_Signal \
    ((uint32_t)0x00000700)                                /* PdelayRespFollowUp message (peer-to-peer transparent clock) or Signaling message \
...
#define IS_ETH_DMAPTPRxDESC_GET_EXTENDED_FLAG(FLAG) \
    (((FLAG) == ETH_DMAPTPRxDesc_PTPV) || ((FLAG) == ETH_DMAPTPRxDesc_PTPFT) || \
        ((FLAG) == ETH_DMAPTPRxDesc_PTPMT) || ((FLAG) == ETH_DMAPTPRxDesc_IPV6PR) || \
        ((FLAG) == ETH_DMAPTPRxDesc_IPV4PR) || ((FLAG) == ETH_DMAPTPRxDesc_IPCB) || \
        ((FLAG) == ETH_DMAPTPRxDesc_IPPE) || ((FLAG) == ETH_DMAPTPRxDesc_IPHE) || ((FLAG) == ETH_DMAPTPRxDesc_IPPT))
...
#define ETH_DMA_ReceiveProcess_Queuing \
    ((uint32_t)0x000E0000) /*!< Running - queuing the receive frame into host memory */
...
#define IS_ETH_PTP_GET_FLAG(FLAG) \
    (((FLAG) == ETH_PTP_FLAG_TSARU) || ((FLAG) == ETH_PTP_FLAG_TSITE) || ((FLAG) == ETH_PTP_FLAG_TSSTU) || \
        ((FLAG) == ETH_PTP_FLAG_TSSTI) || ((FLAG) == ETH_PTP_FLAG_TSTTR) || ((FLAG) == ETH_PTP_FLAG_TSSO))
...
#define IS_ETH_PTP_TYPE_CLOCK(CLOCK) \
    (((CLOCK) == ETH_PTP_OrdinaryClock) || ((CLOCK) == ETH_PTP_BoundaryClock) || \
        ((CLOCK) == ETH_PTP_EndToEndTransparentClock) || ((CLOCK) == ETH_PTP_PeerToPeerTransparentClock))
/**
 * @brief  ETHERNET snapshot
 */
#define ETH_PTP_SnapshotMasterMessage \
...
#define IS_ETH_PTP_SNAPSHOT(SNAPSHOT) \
    (((SNAPSHOT) == ETH_PTP_SnapshotMasterMessage) || ((SNAPSHOT) == ETH_PTP_SnapshotEventMessage) || \
        ((SNAPSHOT) == ETH_PTP_SnapshotIPV4Frames) || ((SNAPSHOT) == ETH_PTP_SnapshotIPV6Frames) || \
        ((SNAPSHOT) == ETH_PTP_SnapshotPTPOverEthernetFrames) || ((SNAPSHOT) == ETH_PTP_SnapshotAllReceivedFrames))
...
#ifdef USE_ENHANCED_DMA_DESCRIPTORS
FlagStatus ETH_GetDMAPTPRxDescExtendedFlagStatus(ETH_DMADESCTypeDef* DMAPTPRxDesc,
