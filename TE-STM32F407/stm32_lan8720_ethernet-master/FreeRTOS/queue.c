...
typedef struct QueueDefinition {
    ...
    volatile BaseType_t xTxLock; /*< Stores the number of items transmitted to the queue (added to the queue) while the
    ...
    const uint8_t ucQueueType)
{
   ...
   configUSE_TRACE_FACILITY not be set to 1. */
    ...
    if(pcAllocatedBuffer != NULL) {
        pxNewQueue = (Queue_t*)pcAllocatedBuffer; /*lint !e826 MISRA The buffer cannot be to small because it was
                                                   dimensioned by sizeof( Queue_t ) + xQueueSizeInBytes. */
