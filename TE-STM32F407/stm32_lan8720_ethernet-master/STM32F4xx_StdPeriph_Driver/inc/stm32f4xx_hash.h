...
#   define IS_HASH_ALGOSELECTION(ALGOSELECTION) \
      (((ALGOSELECTION) == HASH_AlgoSelection_SHA1) || ((ALGOSELECTION) == HASH_AlgoSelection_MD5))
/**
 * @}
 */
...
#   define HASH_FLAG_DINIS \
      ((uint16_t)0x0001) /*!< 16 locations are free in the DIN : A new block can be entered into the input buffer.*/
...
#   define HASH_FLAG_DINNE \
      ((uint16_t)0x1000) /*!< DIN not empty : The input buffer contains at least one word of data */
