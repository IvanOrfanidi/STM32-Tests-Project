...
#define RTC_FLAGS_MASK \
   ((uint32_t)(RTC_FLAG_TSOVF | RTC_FLAG_TSF | RTC_FLAG_WUTF | RTC_FLAG_ALRBF | RTC_FLAG_ALRAF | RTC_FLAG_INITF | \
               RTC_FLAG_RSF | RTC_FLAG_INITS | RTC_FLAG_WUTWF | RTC_FLAG_ALRBWF | RTC_FLAG_ALRAWF | RTC_FLAG_TAMP1F | \
...
void RTC_SetAlarm(uint32_t RTC_Format, uint32_t RTC_Alarm, RTC_AlarmTypeDef* RTC_AlarmStruct)
{
   ...
   if (RTC_Format != RTC_Format_BIN)
   {
      tmpreg = (((uint32_t)(RTC_AlarmStruct->RTC_AlarmTime.RTC_Hours) << 16) |
                ...
                ((uint32_t)RTC_AlarmStruct->RTC_AlarmDateWeekDaySel) | ((uint32_t)RTC_AlarmStruct->RTC_AlarmMask));
   }
   else
   {
