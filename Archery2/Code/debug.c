...
void cout(const char* fmt_ptr, ...)
{
    ...
    if(osKernelRunning()) {                              //Если запущена OS
        if(xSemaphoreTake(sBinSemDbg, 1) != pdTRUE) {    //Проверка занетостити отладочного интерфейса
#ifdef USE_RESERVE_BUFF
            if(ptrReserveBuf)
...
{
#ifdef USE_RESERVE_BUFF
    if(ptrReserveBuf) {    //Если есть данные для догрузки, то выведем их
        cout(ptrReserveBuf);
...
{
#if(DBG_TX_BUFFER_SIZE)
    if(dbg_tx_coun) {    //Если есть что отправить, то передаем, иначе выключаем передатчик
        USART_SendData(UART_DBG_INTRERFACE, dbg_tx_buf[dbg_tx_rd_index++]);
        if(dbg_tx_rd_index >= dbg_tx_coun) {    //Буфер пуст
            dbg_tx_coun = NULL;
...
#ifdef USE_RESERVE_BUFF
                if(ptrReserveBuf) {    //Если есть указатель резервного буфера, то поставим мьютекс догрузки.
