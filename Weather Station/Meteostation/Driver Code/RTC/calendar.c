...
uint32_t Date2Sec(const RTC_t* pSrc)
{
    ...
    if(year < 70) {
        ...
        for(tmp = 0; tmp < year; ulTimeInSec += 86400L * (365L + visocosn(tmp)), tmp++)
            ;
    }
    else {
        for(tmp = 70; tmp < year; ulTimeInSec += 86400L * (365L + visocosn(tmp)), tmp++)
            ;
    }

    for(tmp = 0; tmp < pSrc->month; ulTimeInSec += (86400L * (mon_len[tmp] + ((tmp == 2) ? visocosn(year) : 0))), tmp++)
        ;
