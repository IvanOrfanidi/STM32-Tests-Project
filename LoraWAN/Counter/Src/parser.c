...
static int parseConfigured(uint8_t* pInData, uint8_t* pOutData, uint8_t lenOut)
{
   ...
   {   //проверка корретности длины буфера
   ...
   if (reg_true)
   {   // запрос на получение конфигурации(интервал в часах, время в timestamp и напряжение питания батареи)
