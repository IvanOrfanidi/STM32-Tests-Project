   ...
   */
   ...
   for (i = 0; i < 50; i++)
   {
      if (satsarr[i] > -1)
      {
         ...
         if (sat_count > 0)
         {
            strcat(strcpy(sat_nums, sat_nums), sat_num);
         }
         else
         {
            strcpy(sat_nums, sat_num);
         }
         ...
         LCD_DrawLine(
            line_from_x + (line_count * 24) + 1, line_from_y - satsarr[i] + 1, satsarr[i] + 1, LCD_DIR_VERTICAL);
         sat_count++;
         line_count++;
      }
   }
