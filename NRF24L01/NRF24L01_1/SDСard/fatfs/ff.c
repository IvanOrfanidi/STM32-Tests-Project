...
static FRESULT move_window(FATFS* fs, DWORD sector) /* Move to zero only writes back dirty window */
{
...
static FRESULT dir_seek(DIR* dj, WORD idx)
{
    DWORD clst;
    WORD ic;
...
static _Bool cmp_lfn(WCHAR* lfnbuf, BYTE* dir)
{
    WCHAR wc;
...
static _Bool pick_lfn(WCHAR* lfnbuf, BYTE* dir)
{
    WCHAR wc;
...
/*-----------------------------------------------------------------------*/
#if _USE_LFN
void gen_numname(BYTE* dst, const BYTE* src, const WCHAR* lfn, WORD num)
{
    char ns[8];
    int i, j;
