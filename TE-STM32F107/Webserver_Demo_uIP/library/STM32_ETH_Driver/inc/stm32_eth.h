...
typedef struct
{
    ...
    */
    ...
    uint32_t ETH_ReceiveOwn;               /*!< Enable/disable the reception of frames when the TX_EN signal is asserted in Half-Duplex
                               mode */
    ...
    uint32_t ETH_ChecksumOffload;          /*!< Enable/disable the calculation of complement sum of all received Ethernet frame
                                    payloads */
    uint32_t ETH_RetryTransmission;        /*!< Enable/disable the MAC attempt retries transmission, based on the settings of
                                      BL, when a colision occurs (Half-Duplex mode) */
    ...
    uint32_t ETH_PassControlFrames;        /*!< Selects None/All/FilterPass of all control frames (including unicast and
                                      multicast PAUSE frames) */
    uint32_t ETH_BroadcastFramesReception; /*!< Enable/disable reception of Broadcast Frames */
    uint32_t ETH_DestinationAddrFilter;    /*!< Selects EnableNormal/EnableInverse destination filter for both unicast and
                                          multicast frames */
    uint32_t ETH_PromiscuousMode;          /*!< Enable/disable Promiscuous Mode */
    uint32_t ETH_MulticastFramesFilter;    /*!< Selects the Multicast Frames filter:
                                          None/HashTableFilter/PerfectFilter/PerfectHashTableFilter */
    uint32_t ETH_UnicastFramesFilter;      /*!< Selects the Unicast Frames filter:
    ...
    uint32_t
        ETH_PauseTime;                    /*!< This field holds the value to be used in the Pause Time field in the transmit control frame */
    uint32_t ETH_ZeroQuantaPause;         /*!< Enable/disable the automatic generation of Zero-Quanta Pause Control frames */
    uint32_t ETH_PauseLowThreshold;       /*!< This field configures the threshold of the PAUSE to be checked for automatic
                                      retransmission of PAUSE Frame */
    uint32_t ETH_UnicastPauseFrameDetect; /*!< Enable/disable MAC to detect the Pause frames (with MAC Address0 unicast
                                            address and unique multicast address) */
    uint32_t ETH_ReceiveFlowControl;      /*!< Enable/disable the MAC to decode the received Pause frame and disable its
                                       transmitter for a specified (Pause Time) time */
    uint32_t ETH_TransmitFlowControl;     /*!< Enable/disable the MAC to transmit Pause frames (Full-Duplex mode) or the MAC
    ...
    */
    ...
    uint32_t ETH_ForwardUndersizedGoodFrames; /*!< Enable/disable Rx FIFO to forward Undersized frames (frames with no
    ...
    uint32_t ETH_SecondFrameOperate;          /*!< Enable/disable the DMA process of a second frame of Transmit data even before
                                       status for first frame is obtained */
    ...
    uint32_t
        ETH_RxDMABurstLength; /*!< Indicate the maximum number of beats to be transferred in one Rx DMA transaction */
    uint32_t
        ETH_TxDMABurstLength;          /*!< Indicate the maximum number of beats to be transferred in one Tx DMA transaction */
    uint32_t ETH_DescriptorSkipLength; /*!< Specifies the number of word to skip between two unchained descriptors (Ring
                                         mode) */
...
#define ETH_DMATxDesc_EC \
    ((uint32_t)0x00000100)                      /*!< Excessive Collision: transmission aborted after 16 collisions \ \
                              */
...
#define ETH_DMARxDesc_DBE \
    ((uint32_t)0x00000004)                      /*!< Dribble bit error: frame contains non int multiple of 8 bits  */
...
#define IS_ETH_PHY_REG(REG) (((REG) == PHY_BCR) || ((REG) == PHY_BSR) || ((REG) == PHY_SR))
...
#define IS_ETH_CHECKSUM_OFFLOAD(CMD) \
    (((CMD) == ETH_ChecksumOffload_Enable) || ((CMD) == ETH_ChecksumOffload_Disable))
...
#define IS_ETH_PROMISCUOUS_MODE(CMD) \
    (((CMD) == ETH_PromiscuousMode_Enable) || ((CMD) == ETH_PromiscuousMode_Disable))
...
#define IS_ETH_MULTICAST_FRAMES_FILTER(FILTER) \
    (((FILTER) == ETH_MulticastFramesFilter_PerfectHashTable) || \
        ((FILTER) == ETH_MulticastFramesFilter_HashTable) || ((FILTER) == ETH_MulticastFramesFilter_Perfect) || \
...
#define IS_ETH_DMA_TXDESC_SEGMENT(SEGMENT) \
    (((SEGMENT) == ETH_DMATxDesc_LastSegment) || ((SEGMENT) == ETH_DMATxDesc_FirstSegment))
...
#define IS_ETH_SECOND_FRAME_OPERATE(CMD) \
    (((CMD) == ETH_SecondFrameOperate_Enable) || ((CMD) == ETH_SecondFrameOperate_Disable))
...
#define IS_ETH_MMC_REGISTER(REG) \
    (((REG) == ETH_MMCCR) || ((REG) == ETH_MMCRIR) || ((REG) == ETH_MMCTIR) || ((REG) == ETH_MMCRIMR) || \
        ((REG) == ETH_MMCTIMR) || ((REG) == ETH_MMCTGFSCCR) || ((REG) == ETH_MMCTGFMSCCR) || ((REG) == ETH_MMCTGFCR) || \
        ((REG) == ETH_MMCRFCECR) || ((REG) == ETH_MMCRFAECR) || ((REG) == ETH_MMCRGUFCR))
...
#define IS_ETH_PTP_GET_FLAG(FLAG) \
    (((FLAG) == ETH_PTP_FLAG_TSARU) || ((FLAG) == ETH_PTP_FLAG_TSITE) || ((FLAG) == ETH_PTP_FLAG_TSSTU) || \
...
#define IS_ETH_PTP_REGISTER(REG) \
    (((REG) == ETH_PTPTSCR) || ((REG) == ETH_PTPSSIR) || ((REG) == ETH_PTPTSHR) || ((REG) == ETH_PTPTSLR) || \
        ((REG) == ETH_PTPTSHUR) || ((REG) == ETH_PTPTSLUR) || ((REG) == ETH_PTPTSAR) || ((REG) == ETH_PTPTTHR) || \
        ((REG) == ETH_PTPTTLR))
