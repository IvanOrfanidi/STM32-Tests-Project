...
\
         ...
         /* Saving SP in pxCurrentTCB and then later restoring it (thinking restoring the task) */ \
         /* will just be restoring the interrupt handler, no way!!! */ \
         __asm__ __volatile__("ld.w    r0, sp[9*4]\n\t" /* Read SR in stack */ \
                              "bfextu  r0, r0, 22, 3\n\t" /* Extract the mode bits to R0. */ \
                              "cp.w    r0, 1\n\t" /* Compare the mode bits with supervisor mode(b'001) */ \
...
\
      __asm__ __volatile__(/* Restore ulCriticalNesting variable */ \
                           ...
                           "st.w    r8[0], r0																		\n\t" \
\
                           /* skip PC and SR */ /* do not use SP if interrupts occurs, SP must be left at bottom of \ \
                                                   stack */ \
                           "sub     r7, sp, -10*4																	\n\t" /* Restore \ \
                                                                                                            r8-r12 and \
                                                                                                            \ LR */ \
                           "ldm     r7++, r8-r12, lr																\n\t" \
\
                           /* RETS will take care of the extra PC and SR restore. */ /* So, we have to prepare the \ \
                                                                                        stack for this. */ \
                           "ld.w    r0, r7[-8*4]																	\n\t" /* Read SR */ \
                           "st.w    r7[-2*4], r0																	\n\t" /* Copy SR */ \
                           "ld.w    r0, r7[-7*4]																	\n\t" /* Read PC */ \
