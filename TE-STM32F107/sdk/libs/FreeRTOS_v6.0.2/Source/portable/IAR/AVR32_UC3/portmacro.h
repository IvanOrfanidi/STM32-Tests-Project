...
#define portRESTORE_CONTEXT_OS_INT() \
    { \
        __asm__ __volatile__(/* Restore R0..R7 */ \
            "ldm     sp++, r0-r7																	\n\t" \
\
            /* With the cooperative scheduler, as there is no context switch by interrupt, */ /* there \
                                                                                                                   \ \
                                                                                                                   is \
                                                                                                                   \ \
                                                                                                                   also \
...
\
        __asm__ __volatile__(                                                                                     /* in order to save R0-R7 */ \
            ...
            "stm     --sp, r0-r7																			\n\t" \
\
            /* in order to save R8-R12 and LR */                                                                  /* do not use SP if interrupts occurs, SP must be left \
                                                                   \ at bottom of stack */ \
            "sub     r7, sp,-16*4																			\n\t" /* Copy \
                                                                                                                  \ \
                                                                                                                  PC \ \
...
\
            ...
            "stm     --r7, r8-r12, lr																		\n\t" \
\
            /* Arriving here we have the following stack organizations: */ /* R8..R12, LR, PC, SR, \ \
                                                                                             R0..R7. */ \
\
...
#define portEXIT_SWITCHING_ISR() \
    { \
        ...
        extern volatile void* volatile pxCurrentTCB; \
\
        __asm__ __volatile__(/* Check if INT0 or higher were being handled (case where the OS tick interrupted another */ /* interrupt handler (which was of a higher priority level but decided to lower its priority */ /* level and allow other lower interrupt level to occur). */ /* In this case it's of no use to switch context and restore a new SP because we purposedly */ /* did not previously save SP in its TCB. */ \
            "ld.w    r0, sp[9*4]																			\n\t"                                                                                                                                                                                                                                                                     /* Read SR in stack */ \
            "bfextu  r0, r0, 22, 3																			\n\t"                                                                                                                                                                                                                                                                     /* Extract the mode bits to R0. */ \
            "cp.w    r0, 1																					\n\t"                                                                                                                                                                                                                                                                     /* Compare the mode bits with supervisor mode(b'001) */ \
