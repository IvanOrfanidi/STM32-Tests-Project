...
#define DFU_PACKET_SZE(frq) (uint8_t)(((frq * 2 * 2) / 1000) & 0xFF), (uint8_t)((((frq * 2 * 2) / 1000) >> 8) & 0xFF)
...
static uint8_t USBD_DFU_EP0_TxReady(USBD_HandleTypeDef* pdev)
{
   ...
   if (hdfu->dev_state == DFU_STATE_DNLOAD_BUSY)
   {
      ...
      else if (hdfu->wblock_num > 1)
      {
         ...
         if (((USBD_DFU_MediaTypeDef*)pdev->pUserData)->Write(hdfu->buffer.d8, (uint8_t*)addr, hdfu->wlength) !=
             USBD_OK)
         {
            return USBD_FAIL;
         }
   ...
   if (req->wLength > 0)
   {
      if ((hdfu->dev_state == DFU_STATE_IDLE) || (hdfu->dev_state == DFU_STATE_UPLOAD_IDLE))
      {
         ...
         else if (hdfu->wblock_num > 1)
         {
            ...
            /* Send the status data over EP0 */
            USBD_CtlSendData(pdev, phaddr, hdfu->wlength);
         }
