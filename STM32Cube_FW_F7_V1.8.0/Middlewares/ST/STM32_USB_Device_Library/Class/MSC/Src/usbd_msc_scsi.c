...
static int8_t SCSI_ProcessRead(USBD_HandleTypeDef* pdev, uint8_t lun)
{
   ...
   if (((USBD_StorageTypeDef*)pdev->pUserData)
          ->Read(lun, hmsc->bot_data, hmsc->scsi_blk_addr / hmsc->scsi_blk_size, len / hmsc->scsi_blk_size) < 0)
   {
...
static int8_t SCSI_ProcessWrite(USBD_HandleTypeDef* pdev, uint8_t lun)
{
   ...
   if (((USBD_StorageTypeDef*)pdev->pUserData)
          ->Write(lun, hmsc->bot_data, hmsc->scsi_blk_addr / hmsc->scsi_blk_size, len / hmsc->scsi_blk_size) < 0)
   {
