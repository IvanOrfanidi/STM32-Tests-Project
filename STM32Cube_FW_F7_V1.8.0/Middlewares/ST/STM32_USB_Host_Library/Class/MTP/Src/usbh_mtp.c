      ...
      if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceClass == USB_MTP_CLASS)
      {
         for (endpoint = 0; endpoint < USBH_MAX_NUM_ENDPOINTS; endpoint++)
         {
            if (((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bEndpointAddress & 0x80) == 0) &&
                (phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].wMaxPacketSize > 0) &&
                ((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bmAttributes & USBH_EP_BULK) ==
                 USBH_EP_BULK))
            {
               return endpoint;
            }
...
static USBH_StatusTypeDef USBH_MTP_Process(USBH_HandleTypeDef* phost)
{
   ...
   case MTP_GETSTORAGEINFO:
      ...
      if (status == USBH_OK)
      {
         USBH_UsrLog("Volume#%lu: %s   [%s]",
                     MTP_Handle->current_storage_unit,
                                          ...
                                          uint32_t* numobs)
{
   ...
   if ((storage_idx < MTP_Handle->info.storids.n) && (MTP_Handle->is_ready))
   {
      while ((status = USBH_PTP_GetNumObjects(
                 phost, MTP_Handle->info.storids.Storage[storage_idx], objectformatcode, associationOH, numobs)) ==
             USBH_BUSY)
      {
                                             ...
                                             PTP_ObjectHandlesTypedef* objecthandles)
{
   ...
   {
      while (
         (status = USBH_PTP_GetObjectHandles(
             phost, MTP_Handle->info.storids.Storage[storage_idx], objectformatcode, associationOH, objecthandles)) ==
         USBH_BUSY)
      {
         if (((phost->Timer - timeout) > 5000) || (phost->device.is_connected == 0))
         {
                                             ...
                                             uint32_t* len)
{
   ...
   if (MTP_Handle->is_ready)
   {
      while ((status = USBH_PTP_GetPartialObject(phost, handle, offset, maxbytes, object, len)) == USBH_BUSY)
      {
         if (((phost->Timer - timeout) > 5000) || (phost->device.is_connected == 0))
         {
            return USBH_FAIL;
                                                    ...
                                                    uint16_t* props)
{
   ...
   if (MTP_Handle->is_ready)
   {
      while ((status = USBH_PTP_GetObjectPropsSupported(phost, ofc, propnum, props)) == USBH_BUSY)
      {
         if (((phost->Timer - timeout) > 5000) || (phost->device.is_connected == 0))
         {
            return USBH_FAIL;
