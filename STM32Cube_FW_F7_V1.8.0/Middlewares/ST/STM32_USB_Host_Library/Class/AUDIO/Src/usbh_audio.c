...
static USBH_StatusTypeDef USBH_AUDIO_SetEndpointControls(USBH_HandleTypeDef* phost, uint8_t Ep, uint8_t* buff);
...
static USBH_StatusTypeDef USBH_AUDIO_ClassRequest(USBH_HandleTypeDef* phost)
{
   ...
   case AUDIO_REQ_SET_DEFAULT_IN_INTERFACE:
      if (AUDIO_Handle->microphone.supported == 1)
      {
         req_status = USBH_SetInterface(phost, AUDIO_Handle->microphone.interface, 0);
   ...
   case AUDIO_REQ_SET_DEFAULT_OUT_INTERFACE:
      if (AUDIO_Handle->headphone.supported == 1)
      {
         req_status = USBH_SetInterface(phost, AUDIO_Handle->headphone.interface, 0);
   ...
   case AUDIO_REQ_SET_IN_INTERFACE:
      ...
      {
         req_status =
            USBH_SetInterface(phost, AUDIO_Handle->microphone.interface, AUDIO_Handle->microphone.AltSettings);
   ...
   case AUDIO_REQ_SET_OUT_INTERFACE:
      if (AUDIO_Handle->headphone.supported == 1)
      {
         req_status = USBH_SetInterface(phost, AUDIO_Handle->headphone.interface, AUDIO_Handle->headphone.AltSettings);
...
static USBH_StatusTypeDef USBH_AUDIO_HandleCSRequest(USBH_HandleTypeDef* phost)
{
   ...
   cs_status = USBH_AUDIO_CSRequest(phost, AUDIO_Handle->temp_feature, AUDIO_Handle->temp_channels);
   ...
   for (terminalIndex = 0; terminalIndex < AUDIO_Handle->class_desc.InputTerminalNum; terminalIndex++)
   {
      if (LE16(AUDIO_Handle->class_desc.cs_desc.InputTerminalDesc[terminalIndex]->wTerminalType) == 0x201)
      {
         ...
         AUDIO_Handle->microphone.asociated_channels =
            AUDIO_Handle->class_desc.cs_desc.InputTerminalDesc[terminalIndex]->bNrChannels;
         break;
      }
   }
   ...
   case AUDIO_CONTROL_INIT:
      if ((phost->Timer & 1) == 0)
      {
         ...
         USBH_InterruptReceiveData(
            phost, (uint8_t*)(AUDIO_Handle->mem), AUDIO_Handle->control.EpSize, AUDIO_Handle->control.Pipe);
...
static USBH_StatusTypeDef USBH_AUDIO_OutputStream(USBH_HandleTypeDef* phost)
{
   ...
   case AUDIO_PLAYBACK_SET_EP_FREQ:
      ...
      buff =
         (uint8_t*)AUDIO_Handle->class_desc.as_desc[AUDIO_Handle->headphone.asociated_as].FormatTypeDesc->tSamFreq[0];
...
static USBH_StatusTypeDef USBH_AUDIO_Transmit(USBH_HandleTypeDef* phost)
{
   ...
   case AUDIO_DATA_START_OUT:
      ...
      if ((phost->Timer & 1) == 0)
      {
         ...
         USBH_IsocSendData(
            phost, AUDIO_Handle->headphone.buf, AUDIO_Handle->headphone.frame_length, AUDIO_Handle->headphone.Pipe);
   ...
   case AUDIO_DATA_OUT:
          ...
          ((phost->Timer - AUDIO_Handle->headphone.timer) >= AUDIO_Handle->headphone.Poll))
      {
         ...
         {
            USBH_IsocSendData(
               phost, AUDIO_Handle->headphone.cbuf, AUDIO_Handle->headphone.frame_length, AUDIO_Handle->headphone.Pipe);
                                           ...
                                           uint8_t BitPerSample)
{
   ...
   if (phost->gState == HOST_CLASS)
   {
      ...
      if (AUDIO_Handle->play_state == AUDIO_PLAYBACK_IDLE)
      {
         ...
         {
            freq_min =
               LE24(AUDIO_Handle->class_desc.as_desc[AUDIO_Handle->headphone.asociated_as].FormatTypeDesc->tSamFreq[0]);
            freq_max =
               LE24(AUDIO_Handle->class_desc.as_desc[AUDIO_Handle->headphone.asociated_as].FormatTypeDesc->tSamFreq[1]);
         ...
         {
            num_supported_freq =
               (AUDIO_Handle->class_desc.as_desc[AUDIO_Handle->headphone.asociated_as].FormatTypeDesc->bLength - 8) / 3;

            for (index = 0; index < num_supported_freq; index++)
            {
               if (SampleRate == LE24(AUDIO_Handle->class_desc.as_desc[AUDIO_Handle->headphone.asociated_as]
...
static USBH_StatusTypeDef USBH_AUDIO_SetControlAttribute(USBH_HandleTypeDef* phost, uint8_t attrib)
{
   ...
   if (AUDIO_SetVolume(
          phost, AUDIO_Handle->temp_feature, AUDIO_Handle->temp_channels, AUDIO_Handle->headphone.attribute.volume) !=
       USBH_BUSY)
   {
      if (AUDIO_Handle->temp_channels == 1)
      {
...
USBH_StatusTypeDef USBH_AUDIO_SetVolume(USBH_HandleTypeDef* phost, AUDIO_VolumeCtrlTypeDef volume_ctl)
{
   ...
   if ((volume_ctl == VOLUME_UP) || (volume_ctl == VOLUME_DOWN))
   {
      if (phost->gState == HOST_CLASS)
      {
         ...
         if (AUDIO_Handle->play_state == AUDIO_PLAYBACK_PLAY)
         {
            AUDIO_Handle->control_state = (volume_ctl == VOLUME_UP) ? AUDIO_CONTROL_VOLUME_UP
...
static USBH_StatusTypeDef AUDIO_SetVolume(USBH_HandleTypeDef* phost, uint8_t feature, uint8_t channel, uint16_t volume)
{
   ...
   status = USBH_AC_SetCur(phost, UAC_FEATURE_UNIT, feature, VOLUME_CONTROL, channel, 2);
