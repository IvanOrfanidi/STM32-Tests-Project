...
#      define WM_SUPPORT_TRANSPARENCY \
         1 /* Should be defined outside of GUI_WINSUPPORT because of '#if GUI_WINSUPPORT && WM_SUPPORT_TRANSPARENCY' \ \
              in some files */
#   endif
...
#      define WM_DELETE \
         11 /* Delete (Destroy) command: This tells the client to free its data strutures since the window \ \
...
#      define WM_CF_HASTRANS \
         (1UL << 0) /* Has transparency. Needs to be defined for windows which do not fill the entire \ \
...
#      define WM_CF_ANCHOR_RIGHT \
         (1UL \
          << 7) /* Right anchor ...  If parent is resized, distance to right  will remain const (left is default) \ \
                 */
#      define WM_CF_ANCHOR_BOTTOM \
         (1UL \
          << 8) /* Bottom anchor ... If parent is resized, distance to bottom will remain const (top  is default) \ \
                 */
#      define WM_CF_ANCHOR_LEFT \
         (1UL \
          << 9) /* Left anchor ...   If parent is resized, distance to left   will remain const (left is default) \ \
                 */
#      define WM_CF_ANCHOR_TOP \
         (1UL \
          << 10) /* Top anchor ...    If parent is resized, distance to top    will remain const (top  is default) */

#      define WM_CF_CONST_OUTLINE \
         (1UL << 11) /* Constant outline. This is relevant for transparent windows only. If a window is transparent \ \
                        and does not have a constant outline, its background is invalided instead of the window \ \
                        itself. This causes add. computation time when redrawing. */
#      define WM_CF_LATE_CLIP (1UL << 12)
#      define WM_CF_MEMDEV_ON_REDRAW (1UL << 13)

#      define WM_SF_INVALID_DRAW (1UL << 14)
#      define WM_SF_DELETE \
         (1UL << 15) /* Marks the window to be deleted within WM_Exec() when no callback routine is executed */
...
struct WM_MESSAGE
{
   ...
   union
   {
      const void* p; /* Some messages need more info ... Pointer is declared "const" because some systems (M16C) have 4
                        byte const, byte 2 byte default ptrs */
      int v;
      GUI_COLOR Color;
   } Data;
};
...
int WM_CreateTimer(WM_HWIN hWin,
                   int UserID,
                   int Period,
...
#      else
WM_HMEM WM_CreateTimer(WM_HWIN hWin,
                       int UserID,
                       int Period,
                       int Mode); /* not to be documented (may change in future version) */
...
void WM_GetInsideRectExScrollbar(WM_HWIN hWin,
                                 GUI_RECT* pRect); /* not to be documented (may change in future version) */
