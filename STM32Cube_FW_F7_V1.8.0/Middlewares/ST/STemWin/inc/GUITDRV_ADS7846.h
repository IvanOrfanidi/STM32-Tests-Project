...
typedef struct
{
   ...
   U16(*pfGetResult)
   (void);   // Retrieves the result of the AD conversion. 4 dummy bytes have to be shifted out to the left.
   ...
   //
   int PressureMin;   // Minimum pressure threshold. A measured pressure below this value means we do not have a valid
                      // touch event.
   int PressureMax;   // Maximum pressure threshold. A measured pressure above this value means we do not have a valid
                      // touch event.
   int PlateResistanceX;   // Resistance of the X-plate of the touch screen. This value is needed for calculation of the
                           // touch pressure.
