...
typedef struct GUI_VNC_CONTEXT
{
   ...
   int LayerIndex;
   int BytesPerPixel;
   int BitsPerPixel;   // Note, that from within the VNC server the function LCD_GetBitsBerPixel() can not be used
                       // because the VNC server runs in a separate thread and the device chain can change during the
                       // function call
   //
