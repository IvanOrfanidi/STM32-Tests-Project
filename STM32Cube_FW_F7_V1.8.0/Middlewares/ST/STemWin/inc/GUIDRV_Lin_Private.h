...
#   define DEFAULT_CONTEXT_MEMBERS \
      ...
      int IsVisible; \
      void (*pfFillRect)( \
         int /* LayerIndex */, int /* x0 */, int /* y0 */, int /* x1 */, int /* y1 */, U32 /* PixelIndex */); \
...
#   define DEFAULT_MANAGEMENT_SETDEVFUNC() \
   ...
   case LCD_DEVFUNC_COPYBUFFER: \
      ...
      break; \
   case LCD_DEVFUNC_DRAWBMP_1BPP: \
      pContext->pfDrawBMP1 = (void (*)(int LayerIndex, \
                                       int x, \
                                       int y, \
                                       U8 const* p, \
                                       int Diff, \
                                       ...
                                       int ySize, \
                                       int BytesPerLine, \
                                       const LCD_PIXELINDEX* pTrans))pFunc; \
      break; \
   case LCD_DEVFUNC_DRAWBMP_8BPP: \
      pContext->pfDrawBMP8 = (void (*)(int LayerIndex, \
                                       int x, \
                                       int y, \
                                       U8 const* p, \
                                       int xSize, \
...
#   if (LCD_MIRROR == 2)
#      define MIRROR(x) \
         x = ((x & 0x000000ffUL) << 8) | ((x & 0x0000ff00UL) >> 8) | ((x & 0x00ff0000UL) << 8) | \
             ((x & 0xff000000UL) >> 8)
#   else
#      define MIRROR(x) \
         x = ((x & 0x000000ffUL) << 24) | ((x & 0x0000ff00UL) << 8) | ((x & 0x00ff0000UL) >> 8) | \
             ((x & 0xff000000UL) >> 24)
#   endif
