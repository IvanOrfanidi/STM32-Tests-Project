...
#      ifdef USE_SRP
          ...
          (pcb->eap.es_server.ea_peerlen - SRP_PSEUDO_LEN) * 3 / 4 < sizeof(secbuf))
      {
         BZERO(&bs, sizeof(bs));
         plen = b64dec(
            &bs, pcb->eap.es_server.ea_peer + SRP_PSEUDO_LEN, pcb->eap.es_server.ea_peerlen - SRP_PSEUDO_LEN, secbuf);
         toffs = 0;
         for (i = 0; i < 5; i++)
         {
      ...
      if (get_srp_secret(pcb->eap.es_unit, pcb->eap.es_server.ea_peer, pcb->eap.es_server.ea_name, (char*)secbuf, 1) !=
          0)
      {
