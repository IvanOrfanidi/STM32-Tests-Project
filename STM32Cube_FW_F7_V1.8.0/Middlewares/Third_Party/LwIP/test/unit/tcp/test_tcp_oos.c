...
static int tcp_oos_count(struct tcp_pcb* pcb)
{
   ...
   (PBUF_POOL_BUFSIZE >= \
    (TCP_MSS + PBUF_LINK_ENCAPSULATION_HLEN + PBUF_LINK_HLEN + PBUF_IP_HLEN + PBUF_TRANSPORT_HLEN))
...
START_TEST(test_tcp_recv_ooseq_overrun_rxwin)
{
   ...
   for (i = TCP_MSS, k = 0; i < TCP_WND; i += TCP_MSS, k++)
   {
      ...
      struct pbuf* p =
         tcp_create_rx_segment(pcb, &data_full_wnd[TCP_MSS * (k + 1)], TCP_MSS, TCP_MSS * (k + 1), 0, TCP_ACK);
      EXPECT_RET(p != NULL);
      /* pass the segment to tcp_input */
