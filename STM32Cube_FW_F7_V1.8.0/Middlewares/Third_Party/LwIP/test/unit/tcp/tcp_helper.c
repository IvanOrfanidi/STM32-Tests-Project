                                ...
                                u8_t headerflags)
{
   return tcp_create_segment_wnd(
      src_ip, dst_ip, src_port, dst_port, data, data_len, seqno, ackno, headerflags, TCP_WND);
}
...
struct pbuf* tcp_create_rx_segment(struct tcp_pcb* pcb,
                                   ...
                                   u8_t headerflags)
{
   return tcp_create_segment(&pcb->remote_ip,
                             &pcb->local_ip,
                             pcb->remote_port,
                             pcb->local_port,
                             data,
...
struct pbuf* tcp_create_rx_segment_wnd(struct tcp_pcb* pcb,
                                       ...
                                       u16_t wnd)
{
   return tcp_create_segment_wnd(&pcb->remote_ip,
                                 &pcb->local_ip,
                                 pcb->remote_port,
                                 pcb->local_port,
                                 data,
