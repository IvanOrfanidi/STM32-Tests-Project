...
static int ssl_write_hello_verify_request(mbedtls_ssl_context* ssl)
{
   ...
   if ((ret = ssl->conf->f_cookie_write(
           ssl->conf->p_cookie, &p, ssl->out_buf + MBEDTLS_SSL_BUFFER_LEN, ssl->cli_id, ssl->cli_id_len)) != 0)
   {
...
#   if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
   if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECJPAKE)
   {
      ...
      ret = mbedtls_ecjpake_write_round_two(
         &ssl->handshake->ecjpake_ctx, p, end - p, &jlen, ssl->conf->f_rng, ssl->conf->p_rng);
      if (ret != 0)
      {
...
#   if defined(MBEDTLS_KEY_EXCHANGE__SOME__ECDHE_ENABLED)
   ...
   curve_matching_done:
      ...
      if ((ret = mbedtls_ecdh_make_params(&ssl->handshake->ecdh_ctx,
                                          &len,
                                          p,
                                          MBEDTLS_SSL_MAX_CONTENT_LEN - n,
                                          ssl->conf->f_rng,
                                          ssl->conf->p_rng)) != 0)
      {
...
#      endif
   ...
   ret = mbedtls_pk_decrypt(
      mbedtls_ssl_own_key(ssl), p, len, peer_pms, &peer_pmslen, sizeof(peer_pms), ssl->conf->f_rng, ssl->conf->p_rng);
   ...
   *p += 2;
   ...
   if (ret == MBEDTLS_ERR_SSL_UNKNOWN_IDENTITY)
   {
      ...
      if ((ret = mbedtls_ssl_send_alert_message(
              ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_UNKNOWN_PSK_IDENTITY)) != 0)
      {
         return (ret);
      }
