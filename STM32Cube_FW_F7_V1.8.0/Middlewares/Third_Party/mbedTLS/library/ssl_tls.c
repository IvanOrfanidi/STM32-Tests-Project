...
#      endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
         ...
         ret = handshake->tls_prf(handshake->premaster,
                                  handshake->pmslen,
...
#   endif
         ret = handshake->tls_prf(
            handshake->premaster, handshake->pmslen, "master secret", handshake->randbytes, 64, session->master, 48);
      if (ret != 0)
      {
         MBEDTLS_SSL_DEBUG_RET(1, "prf", ret);
         return (ret);
...
#   if defined(MBEDTLS_SSL_HW_RECORD_ACCEL)
   if (mbedtls_ssl_hw_record_init != NULL)
   {
      ...
      if ((ret = mbedtls_ssl_hw_record_init(ssl,
                                            key1,
                                            key2,
                                            transform->keylen,
                                            transform->iv_enc,
...
#   if defined(MBEDTLS_SSL_EXPORT_KEYS)
   if (ssl->conf->f_export_keys != NULL)
   {
      ssl->conf->f_export_keys(
         ssl->conf->p_export_keys, session->master, keyblk, transform->maclen, transform->keylen, iv_copy_len);
   }
#   endif
...
#   if defined(MBEDTLS_ARC4_C) || defined(MBEDTLS_CIPHER_NULL_CIPHER) || \
      (defined(MBEDTLS_CIPHER_MODE_CBC) && (defined(MBEDTLS_AES_C) || defined(MBEDTLS_CAMELLIA_C)))
...
#   if defined(MBEDTLS_CIPHER_MODE_CBC) && (defined(MBEDTLS_AES_C) || defined(MBEDTLS_CAMELLIA_C))
...
#   endif /* MBEDTLS_SSL_HW_RECORD_ACCEL */
   if (!done)
   {
      ...
      MBEDTLS_SSL_DEBUG_BUF(
         4, "output record sent to network", ssl->out_hdr, mbedtls_ssl_hdr_len(ssl) + ssl->out_msglen);
   }
...
static int ssl_parse_record_header(mbedtls_ssl_context* ssl)
{
       ...
       ssl->in_msgtype != MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC && ssl->in_msgtype != MBEDTLS_SSL_MSG_APPLICATION_DATA)
   {
      ...
      if ((ret = mbedtls_ssl_send_alert_message(
              ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL, MBEDTLS_SSL_ALERT_MSG_UNEXPECTED_MESSAGE)) != 0)
      {
         return (ret);
      }
...
#   if defined(MBEDTLS_ECP_C)
static mbedtls_ecp_group_id ssl_preset_suiteb_curves[] = { MBEDTLS_ECP_DP_SECP256R1,
