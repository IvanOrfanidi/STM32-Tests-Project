...
#if !defined(MBEDTLS_ENTROPY_C) || !defined(MBEDTLS_SSL_TLS_C) || !defined(MBEDTLS_SSL_CLI_C) || \
...
#   define USAGE \
      ...
      "    reconnect_hard=%%d   default: 0 (disabled)\n" USAGE_TICKETS USAGE_MAX_FRAG_LEN USAGE_TRUNC_HMAC USAGE_ALPN \
         USAGE_FALLBACK USAGE_EMS USAGE_ETM USAGE_RECSPLIT USAGE_DHMLEN "\n" \
...
#   if defined(MBEDTLS_SSL_CBC_RECORD_SPLITTING)
   if (opt.recsplit != DFL_RECSPLIT)
      mbedtls_ssl_conf_cbc_record_splitting(
         &conf, opt.recsplit ? MBEDTLS_SSL_CBC_RECORD_SPLITTING_ENABLED : MBEDTLS_SSL_CBC_RECORD_SPLITTING_DISABLED);
#   endif
...
#   if defined(MBEDTLS_KEY_EXCHANGE__SOME__PSK_ENABLED)
   if ((ret = mbedtls_ssl_conf_psk(
           &conf, psk, psk_len, (const unsigned char*)opt.psk_identity, strlen(opt.psk_identity))) != 0)
   {
...
#   endif
   ...
   if (opt.nbio == 2)
      ...
      mbedtls_ssl_set_bio(
         &ssl, &server_fd, mbedtls_net_send, mbedtls_net_recv, opt.nbio == 0 ? mbedtls_net_recv_timeout : NULL);
