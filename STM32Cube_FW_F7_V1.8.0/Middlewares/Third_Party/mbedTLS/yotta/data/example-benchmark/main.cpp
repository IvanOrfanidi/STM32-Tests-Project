...
#      if defined(MBEDTLS_GCM_C)
   if (todo.aes_gcm)
   {
      ...
      for (keysize = 128; keysize <= 256; keysize += 64)
      {
         ...
         TIME_AND_TSC(
            title, mbedtls_gcm_crypt_and_tag(&gcm, MBEDTLS_GCM_ENCRYPT, BUFSIZE, tmp, 12, NULL, 0, buf, buf, 16, tmp));
...
#   if defined(MBEDTLS_RSA_C) && defined(MBEDTLS_PEM_PARSE_C) && defined(MBEDTLS_PK_PARSE_C)
   if (todo.rsa)
   {
      ...
      for (curve_info = mbedtls_ecp_curve_list(); curve_info->grp_id != MBEDTLS_ECP_DP_NONE; curve_info++)
      {
         ...
         TIME_PUBLIC(title,
                     ...
                     ret = mbedtls_ecdsa_write_signature(
                        &ecdsa, MBEDTLS_MD_SHA256, buf, curve_info->bit_size, tmp, &sig_len, myrand, NULL));
      ...
      for (curve_info = mbedtls_ecp_curve_list(); curve_info->grp_id != MBEDTLS_ECP_DP_NONE; curve_info++)
      {
         ...
         if (mbedtls_ecdsa_genkey(&ecdsa, curve_info->grp_id, myrand, NULL) != 0 ||
             mbedtls_ecdsa_write_signature(
                &ecdsa, MBEDTLS_MD_SHA256, buf, curve_info->bit_size, tmp, &sig_len, myrand, NULL) != 0)
         {
            mbedtls_exit(1);
         }
         ecp_clear_precomputed(&ecdsa.grp);
         ...
         mbedtls_snprintf(title, sizeof(title), "ECDSA-%s", curve_info->name);
         TIME_PUBLIC(
            title, "verify", ret = mbedtls_ecdsa_read_signature(&ecdsa, buf, curve_info->bit_size, tmp, sig_len));
...
#      if defined(MBEDTLS_ECP_DP_CURVE25519_ENABLED)
      ...
      TIME_PUBLIC(
         "ECDHE-Curve25519", "handshake", ret |= mbedtls_ecdh_gen_public(&ecdh.grp, &ecdh.d, &ecdh.Q, myrand, NULL);
