...
PRIVILEGED_FUNCTION BaseType_t xTaskCreate(
...
PRIVILEGED_FUNCTION TaskHandle_t xTaskCreateStatic(
   TaskFunction_t pxTaskCode,
   const char* const pcName,
...
PRIVILEGED_FUNCTION BaseType_t xTaskCreateRestricted(const TaskParameters_t* const pxTaskDefinition,
 ...
 * \ingroup TaskUtils
 */
PRIVILEGED_FUNCTION char* pcTaskGetName(TaskHandle_t xTaskToQuery); /*lint !e971 Unqualified char types are allowed for
...
PRIVILEGED_FUNCTION void vTaskList(
   char* pcWriteBuffer); /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
