...
#define osThreadDef(name, thread, priority, instances, stacksz) extern osThreadDef_t os_thread_def_##name
...
#else    // define the object
#define osMailQDef(name, queue_sz, type) \
    struct os_mailQ_cb* os_mailQ_cb_##name osMailQDef_t os_mailQ_def_##name = { (queue_sz), \
        sizeof(type), \
        (&os_mailQ_cb_##name) }
#endif
