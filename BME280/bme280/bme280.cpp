...
Bme::Bme(I2C_TypeDef* i2c, uint32_t clock, const Params_t* const params)
{
   ...
   if(BmeInfo.ChipID == BME280_CHIP_ID)
   {
      ...
      if(params) {
         ...
         Params.Standby = BME280_STANDBY_250;
      }
   }
   else {
      // Датчик не читаеться
...
void Bme::Reset()
{
   ...
   while(timeout--)
   {
      uint8_t status = ReadReg(BME280_STATUS);
      if(!(status & 1)) {
         break;
      }
   }
...
bool Bme::ReadCalibration()
{
   ...
   I2C_SendData(I2Cx, BME280_CALIB);   // Send calibration first register address
   timeout = TIMEOUT;
   ...
   Calibration.H2 = (int16_t)(ReadReg(BME280_HUM_CALIB_H2_LSB) << 8 | ReadReg(BME280_HUM_CALIB_H2_MSB));
   Calibration.H3 = ReadReg(BME280_HUM_CALIB_H3);

   const uint16_t h4 = ReadReg(BME280_HUM_CALIB_H4_LSB) << 8 | ReadReg(BME280_HUM_CALIB_H4_MSB);
   const uint16_t h5 = ReadReg(BME280_HUM_CALIB_H5_LSB) << 8 | ReadReg(BME280_HUM_CALIB_H5_MSB);
 ...
 * @param [out] raw_pres - the RAW humidity
 * @retval true -  Success;
 *         false - Fail.
 */
