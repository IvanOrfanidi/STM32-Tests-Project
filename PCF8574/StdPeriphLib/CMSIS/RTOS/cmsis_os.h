...
Definitions that are marked with <b>MUST REMAIN UNCHANGED</b> cannot be altered. Otherwise the CMSIS-RTOS implementation
is no longer compliant to the standard. Note that some functions are optional and need not to be provided by every
...
With <b>\#define osObjectsExternal</b> objects are defined as external symbols. This allows to create a consistent
header file that is used troughtout a project as shown below:
...
present before the header file, the objects are defined as external symbols. A single consistent header file can
therefore be used throughout the whole project.
...
#define osThreadDef(name, priority, instances, stacksz) \
    osThreadDef_t os_thread_def_##name = { (name), (priority), (instances), (stacksz) }
#endif
...
#else    // define the object
#define osTimerDef(name, function) osTimerDef_t os_timer_def_##name = { (function) }
#endif
...
#else    // define the object
#define osPoolDef(name, no, type) osPoolDef_t os_pool_def_##name = { (no), sizeof(type), NULL }
#endif
...
#else    // define the object
#define osMessageQDef(name, queue_sz, type) osMessageQDef_t os_messageQ_def_##name = { (queue_sz), sizeof(type) }
#endif
...
#else    // define the object
#define osMailQDef(name, queue_sz, type) osMailQDef_t os_mailQ_def_##name = { (queue_sz), sizeof(type) }
#endif
