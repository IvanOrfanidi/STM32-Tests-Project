...
const uint8_t egts_crc8table[256] = {
...
const uint16_t egts_crc16table[256] = {
...
uint8_t egts_crc8(uint8_t* lpBlock, uint8_t len)
{
   ...
   while (len--)
   {
      crc = egts_crc8table[crc ^ *lpBlock++];
   }
   return crc;
}
...
uint16_t egts_crc16(uint8_t* pcBlock, uint16_t len)
{
   ...
   while (len--)
   {
      crc = (crc << 8) ^ egts_crc16table[(crc >> 8) ^ *pcBlock++];
   }
   return crc;
}
