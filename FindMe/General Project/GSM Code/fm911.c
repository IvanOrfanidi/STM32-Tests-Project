...
void changeServerFm2iON(void)
{
    ...
    resetConnectError();
    if(GetSleepTimeStandart() > MAX_VAL_SLEEP_TIME_STANDART) {    // Проверка переполнения заданных режимов пробуждений при уходе с 911.
        SetSleepTimeStandart(DEF_TIME_SLEEP_STANDART_DEVICE);
    }
    if(GetSleepTimeFind() > MAX_VAL_SLEEP_TIME_FIND) {    // Проверка переполнения заданных режимов пробуждений при уходе с 911.
...
static int (*ptrGsmFindme911Scheduler[])(char* pOut, int OffsetData) = {
    FrameBuildSystemFm911,
    FrameBuildGpsDataFm911,
    FrameBuildGsmDataFm911,
    FrameBuildAccelDataFm911,
    FrameBuildDeviceDataFm911,
