...
uint32_t additionOtherLbsForFm911(GSM_INFO* pLbsCeng)
{
    ...
    loop(pLbsCeng->count)
    {
        if(!(pLbsCeng->inf->station[i].rxlev)) {    //Фильтруем нулевые данные если всех станций слишком мало.
            pLbsCeng->count = i;
            break;
        }
        DP_GSM("N%d) ", i);
...
uint32_t additionOtherLbsForIon(GSM_INFO* pLbsCeng)
{
    ...
    for(uint8_t n = 0; n < pLbsCeng->count; n++) {
        ...
        for(uint8_t i = index_offset; i < stLbsCnetscan.count; i++) {
            if(stLbsCnetscan.inf->station[i].mcc != 0xFFFF && stLbsCnetscan.inf->station[index].mnc != 0xFF) {    //Проверка, что уже обрабатывали эти данные
...
int CallEventTel(const char* pTelUser)
{
    ...
    if(pTelUser[0] == '8') {    //Если у нас первая цифра '8', то просто скопируем номер,
...
#define LOW_GSM_SIGNAL 10
            if(rssi < LOW_GSM_SIGNAL) {    // Low signal strength
...
SIM_CARD GetNumSim(void)
{
    ...
    loop(MC_COUNT)
    {
        ...
        if(pFindSIM) {
            if(pFindSIM[5] == '1') {    // Проверяем статус выбранной СИМ карты(подключена/удалена)
        ...
        if(pFindSIM) {
            if(pFindSIM[7] == '1') {    // Проверяем статус выбранной СИМ карты(подключена/удалена)
...
SIM_CARD SelectNumSim(SIM_CARD eNumSim)
{
    ...
    if(eNumSim == FIRST_SIMCARD_OK) {    //Выбор первой SIM карты
