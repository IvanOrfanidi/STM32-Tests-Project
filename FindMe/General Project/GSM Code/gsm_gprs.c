...
RET_INFO profile_activate(PROF_CONNECT_SERVER nProf, const char* pNameServ)
{
    ...
    if(nProf == PROF_AGPS_SERVER) {    //Если соединение для SIM800 и метод HTTP GET, то выйдем без открытия, откроем его по другому.
        return RET_GPRS_OK;
    }
    ...
    if(ser_connect_true) {    // если подключен, то делаем дисконект
