...
M_INFO SimCardPin(void)
{
   ...
   {
      if (strcmp(strRamFIRST_SCID, strFIRST_SCID))
      {   //СИМ карты не равны(пользовательская СИМ карта)
   ...
   while (request != M_SIM_READY)
   {
      ...
      if (request == M_SIM_PIN || request == M_SIM_PUK)
      {   //Пин код не подошел, выйдем из запроса
         return request;
      }
      if (++cnt > MC_COUNT)
...
int GetSimScidFromGsmModule(char* pSCID)
{
   ...
   while ((strlen(data_mc.msg[0].str) < SIZE_SCID) && (err_scid))
   {
      ...
      if (data_mc.m_type == M_STRING)
      {
         ...
         if (isdigit(data_mc.msg[0].str[0]))
         {   //Проверим корректный SCID
