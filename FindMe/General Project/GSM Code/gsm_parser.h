...
typedef struct
{
   u8 rxlev;   // Receive level, in decimal format
   u16 mcc;   // Mobile country code, in decimal format
   u8 mnc;   // Mobile network code, in decimal format
   u16 lac;   // Location area code, in hexadecimal format
   u16 cell;   // Cell id, in hexadecimal format
} TS_STATION;

// информация по всем видимым станциям GSM
typedef struct
{
#define SIZE_LBS 10
   TS_STATION station[SIZE_LBS];
   uint8_t ta;
} TS_MOND;
