...
void updateStatusReset(void)
{
   ...
   if (eStatReset > 10)
   {   //Не штатная перезагрузка девайса.
   ...
   if (GetEnableAccelToFind())
   {
      ...
      else
      {
         ...
         {   //Проснулись по нажатию тревожной кнопки
            DPS("<BUTTON_B>\r\n");
...
void UpdateTimeSleepDevice(void)
{
   ...
   if (accel_enable)
   {   //Если включено просыпание от аксел.
      /* Enable WKUP pin 1 */
...
void controlSleepMode(void)
{
   ...
   {   //Если в режиме регистраций
      if (!(GetFlagsControlRegister() & PIN_RSTF))
      {   //Проверяем нажатие кнопки и если кнопка не нажата, то уснем.
         RCC_ClearFlag();
         InitUSART(UART_DBG, DBG_BAUDRATE);
         InitDMA(UART_DBG);
         DPS("\r\n-=D_BAD RESET=-\r\n");
...
void SetTimeSleepDevice(void)
{
   ...
   if (CurSec >= (WkpSec - DELTA_TIME_OFFSET))
   {   // Если текущее время больше времени пробуждения, то сдвигаем время на дельту
