...
int pass_change(u8* pansw, u8* parg, u16 len)
{
    ...
    /* ставим флаг что изменилась конфигурация, что бы отправить пакет конфига (221) */
    extern char g_aucBufDownHttpFirm[];
...
#define CMD_MAX_ANSW_LEN_ECHO 30
    ...
    for(; cmd_start < buf_sz; cmd_start += cmd_stop + uc_delim_len) {
        ...
        if((cmd_stop = findstr(cmd_delimiter[uc_interf], (char*)&pcmd[cmd_start], buf_sz - cmd_start)) ==
            -1)    // Здесь cmd_start не верный, указывает на начало сообщения
            break;
        //ищем параметр переданный в команду
        for(i = 0; i < sizeof(cmd_separator); i++) {
            if((cmd_val = findchar((char*)&pcmd[cmd_start], cmd_separator[i], cmd_stop)) != -1) {    //нашли, сдвинемся на значение
                cmd_val += 1;
                break;
            }
            cmd_val = cmd_stop + 1;
        ...
        if(cmd_indx != FAIL) {
            ...
            if(uc_interf == INTERF_SMS && (flags & SMS_ACCESS_DIS) || (flags & NOT_SUPPORTED_CMD)) {    //нет доступа по смс или чтение параметра запрещено
                ...
                if(cmd_type == USER_FUNC_CMD) {
                    ...
                    switch(cmd_type) {
                        ...
                        case SET_VAL_CMD:
                            if((flags & PASS_EN) && curr_cmd_pass == 0) {    //ошибка доступа, не задан пароль
                                error = CMD_ERR_PASS;
                            }
                            else {
                                if(flags & NUM_MASK) {    //работаем с числами
                                    uint8_t* str_end;
                                    char ucTempNum[15];
                                    char ucTempStr[15];
                                    memset(ucTempNum, 0, sizeof(ucTempNum));
                                    /* Костыль на коректные значения циферь */
                                    int n = 0;
                        ...
                        case GET_VAL_CMD:
                            ...
                            if((flags & READ_PASS) && curr_cmd_pass == 0) {    //ошибка доступа, не задан пароль
                                error = CMD_ERR_PASS;
                            }
                            else if(flags & READ_DIS && ((flags & TYPE_STRING) == 0 || *psrc != 0)) {    //нет доступа по смс или чтение параметра запрещено
                                error = CMD_ERR_ACCESS;
                            }
                            else {
            ...
            if(flags & FLAG_OK) {
                switch(error) {
                    case CMD_PASS_OK:
                        ...
                        break;
                    case 0:
                        /* ставим флаг что изменилась конфигурация, что бы отправить пакет конфига (201) */
