...
const uint8_t _time_days_in_month_internal[2][13] = { { 0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 },
    { 0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 } };

const uint32_t _time_secs_before_month_internal[2][13] = { { 0,
                                                               31UL * SECS_IN_DAY,
                                                               59UL * SECS_IN_DAY,
                                                               90UL * SECS_IN_DAY,
                                                               120UL * SECS_IN_DAY,
...
const uint32_t _time_secs_before_year_internal[] = { ((1970UL - 1970UL) * SECS_IN_YEAR) + (0UL * SECS_IN_DAY),
...
float ConvertLatitudeGpsFindMe(float fLatitude)
{
    uint8_t temp_deg;
    uint8_t temp_min;
    float temp_sec;
    ...
    temp_deg = (uint8_t)((fLatitude / 1000000));                            //находим градусы.
    temp_min = (uint8_t)((fLatitude - (temp_deg * 1000000)) / 10000);       //находим минуты.
    temp_sec = (fLatitude - (temp_deg * 100 + temp_min) * 10000) / 1000;    //находим сек.
    if(temp_deg >= 90) {
        latitude = (temp_deg - 90) + temp_min / 100.0 + temp_sec / 1000.0;
    }
    else {
        latitude = (temp_deg) + temp_min / 100.0 + temp_sec / 1000.0;
        latitude *= -1;
    }
    latitude *= 100;
    return latitude;
...
float ConvertLongitudeGpsFindMe(float fLongitude)
{
    uint8_t temp_deg;
    uint8_t temp_min;
    float temp_sec;
    ...
    temp_deg = (uint8_t)(fLongitude / 1000000);                              //находим градусы.
    temp_min = (uint8_t)((fLongitude - (temp_deg * 1000000)) / 10000);       //находим минуты.
    temp_sec = (fLongitude - (temp_deg * 100 + temp_min) * 10000) / 1000;    //находим сек.
    if(temp_deg >= 180) {
        longitude = (temp_deg - 180) + temp_min / 100.0 + temp_sec / 1000.0;
    }
    else {
        longitude = (temp_deg) + temp_min / 100.0 + temp_sec / 1000.0;
        longitude *= -1;
    }
