...
void Sec2Date(RTC_t* pDest, uint32_t ulSec)
{
   ...
   for (dl = 365L; ulSec >= (dl = 86400L * (365L + visocosn(pDest->year))); ulSec -= dl, pDest->year++)
      ;

   for (pDest->month = 1;
        ulSec >= (dl = 86400L * (mon_len[pDest->month] + ((pDest->month == 2) ? visocosn(pDest->year) : 0)));
        ulSec -= dl, pDest->month++)
      ;
...
uint32_t Date2Sec(const RTC_t* pSrc)
{
   ...
   if (pSrc->year < 70)
   {
      ...
      for (tmp = 0; tmp < pSrc->year; ulTimeInSec += 86400L * (365L + visocosn(tmp)), tmp++)
         ;
   }
   else
   {
      for (tmp = 70; tmp < pSrc->year; ulTimeInSec += 86400L * (365L + visocosn(tmp)), tmp++)
         ;
   }

   for (tmp = 0; tmp < pSrc->month;
        ulTimeInSec += (86400L * (mon_len[tmp] + ((tmp == 2) ? visocosn(pSrc->year) : 0))), tmp++)
      ;
...
void TimeSynchronizationRTC(uint32_t SecGPS)
{
   ...
   if (SecRTC < SecGPS - DELTA_ERR_TIME_RTC || SecRTC > SecGPS + DELTA_ERR_TIME_RTC)
   {
      RTC_t DateRTC;
      Sec2Date(&DateRTC, SecGPS);
