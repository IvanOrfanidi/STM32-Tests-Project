...
uint16_t g_u16IndexReadOffsetUart2 = 0;
...
void InitUSART2(uint32_t baud_rate)
{
   ...
   // GPIO Config //
   GPIO_PinAFConfig(USART2_PORT, USART2_TX_SOURCE, USART2_AF);
   GPIO_PinAFConfig(USART2_PORT, USART2_RX_SOURCE, USART2_AF);
...
void InitUSART3(uint32_t baud_rate)
{
   ...
   // Enable the GPIOs clocks
   RCC_AHBPeriphClockCmd(USART3_PORT_CLK, ENABLE);

   // GPIO Config //
   GPIO_PinAFConfig(USART3_PORT, USART3_TX_SOURCE, USART3_AF);
   GPIO_PinAFConfig(USART3_PORT, USART3_RX_SOURCE, USART3_AF);
...
#if (GPS_ACCURACY_ENABLE)
   "ACCURACY",
#endif
};

_Bool NMEA_CR_END = FALSE;

// Прерывание буфера DMA GPS //
void DMA1_Channel3_IRQHandler(void)
{
   ...
   {
      aucGpsBuffIn[Index] = g_aucRxBufferUSART3[0];
      Index++;

      if (Index > sizeof(aucGpsBuffIn))
      {   //Защита от переполнения буфера.
         Index = 0;
      }

      if (g_aucRxBufferUSART3[0] == '\n')
      {
         NMEA_CR_END = TRUE;
         /* фильтрация не корректных NMEA телеграмм */
         // for(uint8_t i = 0; i < sizeof(strMsgNmeaFilter)/sizeof(void*); i++)
         {
            // if(strstr(aucGpsBuffIn, strMsgNmeaFilter[i]))
            {
...
void DMA1_Channel5_IRQHandler(void)
{
   ...
   {
      // uint8_t ucByte = g_aucRxBufferUSART1[0];
      // DebugHandlerCmd(ucByte);
...
void USART2_Read(char* pData_Usart, uint16_t Len)
{
   ...
   for (index_read = 0; index_read < Len; index_read++)
   {
      pData_Usart[index_read] = g_aucRxBufferUSART2[g_u16IndexReadOffsetUart2];
      g_aucRxBufferUSART2[g_u16IndexReadOffsetUart2] = 0;

      g_u16IndexReadOffsetUart2++;
   }

   if ((RX_BUFFER_SIZE2 - DMA_GetCurrDataCounter(USART2_Rx_DMA_Channel)) <= g_u16IndexReadOffsetUart2)
   {
      DMA_Cmd(USART2_Rx_DMA_Channel, DISABLE);
      DMA_SetCurrDataCounter(USART2_Rx_DMA_Channel, RX_BUFFER_SIZE2);
      g_u16IndexReadOffsetUart2 = 0;
...
void USART_Write(uint8_t NumUSART, const char* pData_Usart, uint16_t Len)
{
   ...
   {
   case 1:
      USART1_Write(pData_Usart, Len);
      break;
   case 2:
      USART2_Write(pData_Usart, Len);
      break;
   case 3:
      USART3_Write(pData_Usart, Len);
      break;
   }
}
...
void USART_Read(uint8_t NumUSART, char* pData_Usart, uint16_t Len)
{
   ...
   {
   case 1:
      USART1_Read(pData_Usart, Len);
      break;
   case 2:
      USART2_Read(pData_Usart, Len);
      break;
   case 3:
      USART3_Read(pData_Usart, Len);
      break;
   }
}
...
void InitUSART(uint8_t NumUSART, uint32_t baud_rate)
{
   ...
   {
   case 1:
      InitUSART1(baud_rate);
      break;
   case 2:
      InitUSART2(baud_rate);
      break;
   case 3:
...
void ReStartDmaUsart1(void)
{
   ...
   USART1_Rx_DMA_Channel->CCR &=
      ~DMA_CCR5_MINC;   // переключаем ля того чтобы сбросить внутреннее смещение дма по буферу
   ...
   osDelay(SLEEP_MS_100);   //ждем пока UART войдет в рабочее стостояние
   while (USART1->SR & USART_SR_RXNE)
   {   //если есть данные в аппаратном буфере UART то
...
void ReStartDmaUsart2(void)
{
   ...
   USART2_Rx_DMA_Channel->CCR &=
      ~DMA_CCR6_MINC;   // переключаем ля того чтобы сбросить внутреннее смещение дма по буферу
   ...
   g_u16IndexReadOffsetUart2 = 0;   //начинаем с чистого буфера - устанавливаем смещение чтения на 0
}

void ReStartDmaUsart3(void)
{
   DMA_Cmd(USART3_Rx_DMA_Channel, DISABLE);   // отключаем канал
   USART3_Rx_DMA_Channel->CCR &=
      ~DMA_CCR3_MINC;   // переключаем ля того чтобы сбросить внутреннее смещение дма по буферу
